...
- `gemini-2.5-flash`
  - `10 RPM`
  - `250,000 TPM`
  - `~20 RPD` (see correction below)
- `gemini-2.5-flash-lite`
  - `15 RPM`
  - `250,000 TPM`
  - `1,000 RPD` (documented; not re-verified against runtime logs)
- `Gemma 3 & 3n`
  - `30 RPM`
  - `15,000 TPM`
  - `14,400 RPD`

> **Correction (2026-05-31):** the `250 RPD` figure for `gemini-2.5-flash` originally recorded here was from Google's docs and is **stale**. The project's own runtime browser logs (see `codex/Progress.md`, 2026-04-27 session) recorded an actual `429 RESOURCE_EXHAUSTED` with `quota id: GenerateRequestsPerDayPerProjectPerModel-FreeTier` and `quota value: 20`. Google has cut the free-tier daily request ceiling. Treat the real free-tier Flash daily cap as **~20 requests/day**, not 250. At 1–2 Flash requests per enhance, that is roughly **10–15 enhances/day** — not a daily-driver budget. Re-verify the live value before relying on it; do not plan against the 250 figure.
...
Practical estimate (corrected 2026-05-31):

- the real free-tier Flash hard wall is `~20 requests/day` (observed `quota value: 20`), not `250/day`
- at 1–2 Flash requests per enhance, that is `~10-15 enhances/day` before fallback takes over
- the earlier `250/day` / `150-200/day` estimates were based on the stale docs figure and are not achievable on the current free tier
...
- at ~20 RPD, free-tier Flash is a demo/trial budget, not a daily-driver budget; a billing-enabled key is required for real daily use
...
### Current Runtime Free Chain

As of the 2026-05-20 critique recovery pass, the repo runtime curates only these OpenRouter free models:

- `nvidia/nemotron-3-super-120b-a12b:free`
- `nvidia/nemotron-3-nano-30b-a3b:free`

Live public OpenRouter catalog check on 2026-05-20:

- `inclusionai/ling-2.6-flash:free`: not present
- `nvidia/nemotron-3-super-120b-a12b:free`: present
- `nvidia/nemotron-3-nano-30b-a3b:free`: present
- `openai/gpt-oss-20b:free`: present, but excluded from the runtime chain
- `openrouter/free`: present, but excluded from the runtime chain
- `google/gemma-4-26b-a4b-it:free`: present, but not part of the curated OpenRouter chain
...
OpenRouter free-router activity and model pages previously showed notable newer free options such as:
...
- `inclusionai/ling-2.6-flash:free` was not present in the public Models API check
- the runtime chain no longer includes Ling
- `openai/gpt-oss-20b:free` remains excluded from the curated runtime chain despite being present
...
### Final OpenRouter Free Chain As Of 2026-05-20
...
- `262,144` context
- strongest current curated free candidate
- present in the 2026-05-20 live catalog check
...
- Ling Flash was absent from the 2026-05-20 public Models API check
- Ling 1T was previously observed as going away on `2026-04-30`
- `openrouter/free` is random routing, not a stable rewrite-quality contract
- GPT-OSS is present but remains excluded until it passes an explicit product/eval decision
...
- scheduled refresh through `chrome.alarms` so catalog refresh survives MV3 service-worker shutdowns
...
- current OpenRouter chain order:
  - `nvidia/nemotron-3-super-120b-a12b:free`
  - `nvidia/nemotron-3-nano-30b-a3b:free`
- excluded from automatic runtime routing:
  - `openrouter/free`
  - `inclusionai/ling-2.6-flash:free`
  - `inclusionai/ling-2.6-1t:free`
  - `openai/gpt-oss-20b:free`
...
- `nvidia/nemotron-3-super-120b-a12b:free` is the current runtime primary but has not passed the live corpus gate yet
- it must pass the eval gate before the OpenRouter chain is described as provider-proven
- if it fails, evaluate `nvidia/nemotron-3-nano-30b-a3b:free` before changing the curated chain
- `inclusionai/ling-2.6-flash:free` is no longer a valid primary target because it was absent from the 2026-05-20 public Models API check
- `openai/gpt-oss-20b:free` remains excluded unless a future explicit product decision reopens it
